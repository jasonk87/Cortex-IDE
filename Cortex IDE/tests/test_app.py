...
def test_get_file_content(client, test_project):
    ...
    res_not_found = client.post('/api/get_file_content', json={'filename': 'nonexistent.txt'})
...
def test_save_file_content(client, test_project):
    ...
    res = client.post('/api/save_file_content', json={'filename': filename, 'content': updated_content})
    assert res.status_code == 200
