...
def test_lint_file_tool_success_with_issues(mock_internal_lint):
    mock_internal_lint.return_value = {
        "success": True,
        "issues": [{"line": 1, "col": 1, "code": "F401", "message": "'os' imported but unused"}]
    }
...
def test_format_file_tool_success_no_changes(mock_internal_format):
    ...
    mock_internal_format.return_value = {"success": True, "formatted_code": original_code}
...
def test_lint_file_tool_api_error(mock_internal_lint):
    mock_internal_lint.return_value = {"success": False, "error": "Syntax error in code"}
