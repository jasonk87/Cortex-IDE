...
import signal
import shutil
from flask import Flask, jsonify, render_template, request, session, send_file, after_this_request
from flask_socketio import SocketIO, join_room # <-- IMPORT 'join_room'
...
def handle_stop_agent():
    ...
    if project_path and project_path in active_agents:
        ...
        print(f"Received stop signal, but no active agent found for project: {project_path}")
...
def handle_disconnect():
    ...
    if project_path and project_path in active_agents:
        active_agents[project_path].stop()
        del active_agents[project_path]
        print(f'Stopped and removed agent for disconnected client in project: {project_path}')
...
def handle_agent_chat(data):
    ...
    if agent.is_running:
        return {"error": "The agent is currently busy. Please wait for the current task to complete."}
...
def download_project():
    ...
    try:
        ...
        zip_archive_path = shutil.make_archive(project_name, 'zip', project_path)
        ...
        def cleanup(response):
            try:
                ...
                print(f"Error cleaning up zip file: {e}") # Log error instead of crashing
            return response
...
def create_file():
    ...
    try:
        ...
        if os.path.exists(file_path): return jsonify({'error': 'File already exists'}), 409
        ...
        return jsonify({'message': f"File '{os.path.basename(file_path)}' created."})
...
def save_file_content():
    ...
    try:
        ...
        filename_to_save = data.get('filename')
        socketio.emit('file_system_updated', {'filename': filename_to_save}, room=project_path)
        return jsonify({'message': f"File '{os.path.basename(file_path)}' saved."})
    except Exception as e: return jsonify({'error': str(e)}), 500
...
def delete_folder():
    ...
    try:
        ...
        if not os.path.isdir(full_path):
            return jsonify({'error': 'The specified path is not a directory.'}), 400

        shutil.rmtree(full_path)
        filename_to_save = data.get('filename')
        socketio.emit('file_system_updated', {'filename': filename_to_save}, room=project_path)
        return jsonify({'message': f"Folder '{folder_path}' and all its contents have been deleted."})
    except Exception as e:
        return jsonify({'error': str(e)}), 500


if __name__ == "__main__":
    socketio.run(app, host='0.0.0.0', debug=False, use_reloader=False, port=5001, allow_unsafe_werkzeug=True)