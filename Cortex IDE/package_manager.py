...
def install_packages():
    ...
    try:
        ...
        if install_result.returncode == 0:
            ...
            return jsonify({
                "success": True, "message": "Packages installed successfully.",
                "stdout": install_result.stdout, "stderr": install_result.stderr
            })
        else:
            if socketio:
            ...
            return jsonify({
                "success": False, "message": "Failed to install packages.",
                "stdout": install_result.stdout, "stderr": install_result.stderr,
                "exit_code": install_result.returncode
            }), 500
    except subprocess.TimeoutExpired:
        if socketio:
            socketio.emit('installation_failed', {'message': 'Package installation timed out.'}, room=project_path)
        return jsonify({"error": "Package installation timed out."}), 408
    except Exception as e:
        if socketio:
            socketio.emit('installation_failed', {'message': f'An unexpected error occurred: {str(e)}'}, room=project_path)
